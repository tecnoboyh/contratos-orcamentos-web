...
export function CompanySwitcher() {
  ...
  const companyOptions = useMemo(() => {
    ...
    return all.filter((item, index, array) => {
      return item?.id && array.findIndex((x) => x.id === item.id) === index;
    });
  }, [company, companies]);
  ...
  function renderCompanyModal() {
    ...
    return createPortal(
      <div className="fixed inset-0 z-[9999] flex min-h-screen items-center justify-center bg-zinc-950/40 px-4 py-6 backdrop-blur-sm">
        <div className="w-full max-w-lg rounded-2xl border border-white/70 bg-white p-5 shadow-[0_24px_80px_rgba(15,23,42,0.24)]">
          <div className="mb-5 flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-zinc-500">
                Cadastre outra empresa para alternar o ambiente no sistema.
              </p>
            </div>
